...
class SpectreSamplingMetrics(nn.Module):
    def __init__(self, loaders, compute_emd, metrics_list):
        super().__init__()
        train_loader, valid_loader, test_loader = loaders
        self.train_graphs = self.loader_to_nx(train_loader)
        self.val_graphs = self.loader_to_nx(valid_loader)
    ...
    def forward(self, generated_graphs: list, test=True):
        ...
        networkx_graphs = []
        output_dict = {'degree': 0, 'clustering': 0, 'orbit': 0, 'VUN': 0, 'U': 0, 'UN': 0}
        ...
        if 'orbit' in self.metrics_list:
            ...
            output_dict['orbit'] = orbit

        if 'sbm' in self.metrics_list or 'planar' in self.metrics_list:
            print("Computing all fractions...")
            validity_func = eval_acc_sbm_graph if 'sbm' in self.metrics_list else eval_acc_planar_graph
            valid_percent = validity_func(networkx_graphs)
            print("Valid percentage: {:.3f}".format(valid_percent))
            # print("Planar percentage: {:.3f}".format(eval_acc_planar_graph(networkx_graphs)))
            if valid_percent > 0:
                ...
                output_dict['VUN'] = fraction_unique_non_isomorphic_valid
                output_dict['U'] = frac_unique
                output_dict['UN'] = frac_unique_non_isomorphic
...
class PlanarSamplingMetrics(SpectreSamplingMetrics):
    def __init__(self, loaders):
        super().__init__(loaders=loaders,
                         compute_emd=False,
                         metrics_list=['degree', 'clustering', 'orbit', 'planar'])
...
class SBMSamplingMetrics(SpectreSamplingMetrics):
    def __init__(self, loaders):
        super().__init__(loaders=loaders,
                         compute_emd=False,
                         metrics_list=['degree', 'clustering', 'orbit', 'sbm'])
